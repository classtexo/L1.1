...
import org.david.task.EnumStatus;
import org.david.task.Issue;
import org.david.task.PersonalTask;
import org.david.task.ProgrammersTask;
import org.david.task.SysTaskDate;
import org.david.task.TaskAbstract;
import org.david.task.TaskHelper;
...
public class TestTaskManager {
	...
	public void main() {
		...
		TaskAbstract taskTemp = new PersonalTask(0, "Prvni ukol",
				"Tyden.length", SysTaskDate.inWeek(), "Jmeno ukolu");
		collectionTasks.add(taskTemp);
		collectionTasks.add(new PersonalTask(0, "Druhy ukol", "den.length",
				SysTaskDate.inDay(), "Jmeno ukolu2"));
		collectionTasks.add(new PersonalTask(0, "Treti ukol", "mesic.length",
				SysTaskDate.dayToday(), "Jmeno ukolu3"));

		Issue[] programmersTaskIssues = { new Issue("SYS3 Exc.error", 1),
				new Issue("Throw error", 2) };
		TaskAbstract ptaskTemp = new ProgrammersTask(0, "Modul 5 - errors",
		...
		ptaskTemp = new ProgrammersTask(0, "Modul 1 - fix", "Week",
