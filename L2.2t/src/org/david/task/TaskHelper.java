...
public class TaskHelper {
	...
	public static void printCollection(List<TaskAbstract> tasks) {
		for (TaskAbstract task : tasks) {
			...
			if (task instanceof ProgrammersTask) {
				System.out.println("Programmers task : ");
			}
	...
	public static boolean clovseSolvedTasks() {
		...
		return true;
		/*
		 * // vezme list tech tasku a co jsou vyresene tak je uzavre // staticke
		 * metody tridy << nastudovat a aplikovat zde
		 */
	}
	...
	public static void printIssuesCount() {
		if (Issue.getIssuesCount() >= 0) {
			System.out
					.println("Issues count : " + (Issue.getIssuesCount() + 1));
		} else {
	...
	public static void sortTasks(List<TaskAbstract> tasks) {
		// TODO trida srovna vstupni list tasku podle budgetu.
	}

	// jeste osefovat konstruktor, figl je, ze toto je neinstancovane, normalni
	...
	// Kdyz schovam konstruktor ze to nikdo neinstancuje a pouzije jen staticke
	// metody.
